...
но знаковость обычного `char` в C отдана на откуп платформе (на Linux
для ARM, RISC-V и LoongArch он беззнаковый, на x86 и на всех платформах
Apple — знаковый), и поле `char` со взведённым старшим битом читается
с неверным знаком. Чтобы получить точные описания, перегенерируйте их
под свой интерпретатор:
