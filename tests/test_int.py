...
class TestInt(unittest.TestCase):
    ...
    def test_caching_ints(self):
        for i in range(-5, 257):
            ...
            assert reflector.ob_base.ob_refcnt.python_value == 4294967295

    def test_int_refcnt(self):
        for i in range(257, 1000):
            reflector = strector.bind_object(i)
            assert reflector.ob_base.ob_refcnt.python_value == getrefcount(i) - 1
    ...
    def test_int_long_value(self):
        ...
        assert reflector.long_value.lv_tag.python_value == 8
        assert reflector.long_value.ob_digit.python_value == ['0x9']
